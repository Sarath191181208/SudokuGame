import pygame
from main import solve, valid, generateBoard, solveBoard
import time
...
class Grid:
    ...
    def solveThis(self):
        boardGrid = deepcopy(self.cubes)
        for i in range(len(boardGrid)):
            for j in range(len(boardGrid)):
                boardGrid[i][j] = boardGrid[i][j].value
        solution = solveBoard(boardGrid)
        x, y = self.selected
        (self.cubes[x][y]).temp = (solution[x][y])
        # print(solution)
